...
public fun ByteArray.securelyRandomize() {
    var pos = 0
    SecureFeed.exportLongs(this, 0, size.ceilDiv(8)) { index, value ->
        var data = value
...
public fun ShortArray.securelyRandomize() {
    var pos = 0
    SecureFeed.exportLongs(this, 0, size.ceilDiv(4)) { index, value ->
        var data = value
        repeat(min(8, size - pos)) {
...
public fun IntArray.securelyRandomize() {
    var pos = 0
    SecureFeed.exportLongs(this, 0, size.ceilDiv(2)) { index, value ->
        var data = value
        repeat(min(8, size - pos)) {
