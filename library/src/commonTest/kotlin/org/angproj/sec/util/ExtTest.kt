...
class ExtTest {
    ...
    @Test
    fun testIncrementWithWrap() {
        // Normal increment
        assertEquals(10L, 5L.incrementWithWrap(5))
        // Wrap around
        assertEquals(0L, Long.MAX_VALUE.incrementWithWrap(1))
        // Large wrap
        assertEquals(5L, (Long.MAX_VALUE - 2).incrementWithWrap(8))
        // Exception for non-positive addition
        assertFailsWith<IllegalArgumentException> { 10L.incrementWithWrap(0) }
        assertFailsWith<IllegalArgumentException> { 10L.incrementWithWrap(-1) }
    }

    @Test
    fun testIncrementWithWrapInt() {
        assertEquals(15L, 10L.incrementWithWrap(5))
        assertEquals(0L, Long.MAX_VALUE.incrementWithWrap(1))
        assertFailsWith<IllegalArgumentException> { 10L.incrementWithWrap(0) }
    }
    ...
    @Test
    fun testToUnitFractionLong() {
        assertEquals(0.0, 0L.toUnitFraction())
        assertEquals(0.5, (1L shl 62).toUnitFraction())
        assertEquals(1.0, Long.MAX_VALUE.toUnitFraction())
        assertEquals(0.0, Long.MIN_VALUE.toUnitFraction())
    }
}